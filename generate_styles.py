"""
Code for dynamically creating style descriptions (and filtering).

This script implements a pipeline for generating and filtering writing style descriptions from text documents.
...
- `--data-dir`: Directory containing the input data in JSONL format. Should look something like:
  ```
  {"documentID":"[DOC_ID]","authorSetIDs":["[AUTHOR_ID]"],"fullText":"[TEXT]","languages":["en"],"lengthWords":[WORD_LEN]}
  ```
...
save_dir = (
    replace_first_word(args["data_dir"], "describe_documents_writing_styles") + ".csv"
)
...
###################
save_dir = os.path.join(
    os.path.dirname(args["data_dir"]), "filtered", "refined_and_aggregated_features.csv"
)
...
if not os.path.exists(save_dir):
    ...
    filtered_df["aggregated_name"] = filtered_df["shortend_attribute_name.v2"].apply(
        lambda x: refine_features_dict[x] if x in refine_features_dict else x
    )
